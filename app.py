...
import db as db_helper #using our db.py file as package
...
def login():
	...
	# if returned list of rows is not empty, the database has an entry for this user (usernames are unique)
		if(len(rows)!=0):
			...
			return render_template('login.html',injection=injection,users=rows,user=user),200 #error with 200 response code
		else:
			error = 'Invalid credentials'
			return render_template('login.html',error=error),401 #error with 401 response code
	else:
